...
function wppugmill_get_compatibility_data() {
	...
	$llms_body_key = 'wppugmill_llms_txt_conflict_check';
	...
	// WP Pugmill's own output starts with "# {Site Name}" and does NOT contain
	...
	<div class="wrap">
				...
				<tr>
					...
					<td>
						<select id="wppugmill_ai_provider" name="wppugmill_ai_provider">
							...
							$saved_provider = ! empty( $api_key ) ? get_option( 'wppugmill_ai_provider', '' ) : '';
		...
		(function() {
			...
			function runTest( afterTest ) {
				if ( isTesting ) { return; }
				isTesting = true;
				status.textContent = '<?php echo esc_js( __( 'Validating\u2026', 'wp-pugmill' ) ); ?>';
				...
				.then( function( res ) {
					if ( res.success ) {
						...
						var msg = ( res.data && res.data.message ) ? res.data.message : '<?php echo esc_js( __( 'Validation failed.', 'wp-pugmill' ) ); ?>';
		...
		$detected_seo         = wppugmill_detected_seo_plugins();
		...
		$disable_breadcrumbs  = (int) get_option( 'wppugmill_disable_breadcrumbs', 0 );
		?>

		<!-- ── SEO Plugin Status ─────────────────────────────────────── -->
		<div style="background:#fff; border:1px solid #ddd; border-radius:8px; padding:20px 24px; margin-top:24px;">
		<h2 style="<?php echo esc_attr( $h2_style ); ?> margin-top:0;"><?php esc_html_e( 'SEO Plugin Status', 'wp-pugmill' ); ?></h2>
		...
		<div style="background:#fef9ec; border-left:4px solid #ffb900; padding:12px 16px; border-radius:0 4px 4px 0; margin-bottom:16px;">
			<strong style="font-size:13px;">⚠ <?php esc_html_e( 'SEO Plugin Detected', 'wp-pugmill' ); ?></strong><br>
			<span style="color:#555; font-size:13px;">
			<?php printf(
				esc_html__( '%s is active. WP Pugmill focuses on AEO — the AI layer that SEO plugins don\'t cover. Review the settings below to avoid duplicate output.', 'wp-pugmill' ),
				esc_html( wppugmill_seo_plugin_names() )
			); ?>
			</span>
		</div>
		...
		<div style="display:flex; align-items:center; gap:10px; padding:12px 16px; background:#f0faf0; border:1px solid #46b450; border-radius:4px;">
			...
			<span style="font-size:13px; color:#1e6e2e; font-weight:600;"><?php esc_html_e( 'No conflicts — no other SEO plugin detected. WP Pugmill is managing all outputs.', 'wp-pugmill' ); ?></span>
		</div>
		<?php endif; ?>
		...
		<p style="<?php echo esc_attr( $p_style ); ?> margin-top:0;">
			<?php if ( $has_seo_plugin ) : ?>
			<?php printf(
				esc_html__( 'The outputs below are also generated by %s. Disable WP Pugmill\'s versions to avoid duplicate tags in your page source. Your AEO content is always saved — only the &lt;head&gt; output changes.', 'wp-pugmill' ),
				esc_html( wppugmill_seo_plugin_names() )
			); ?>
			<?php else : ?>
		...
		<thead>
			<tr style="border-bottom:2px solid #e5e7eb;">
				<th style="text-align:left; padding:8px 0; font-size:12px; color:#6b7280; font-weight:600; width:40%;"><?php esc_html_e( 'Output', 'wp-pugmill' ); ?></th>
				<th style="text-align:left; padding:8px 0; font-size:12px; color:#6b7280; font-weight:600; width:30%;"><?php esc_html_e( 'Handled by', 'wp-pugmill' ); ?></th>
				<th style="text-align:left; padding:8px 0; font-size:12px; color:#6b7280; font-weight:600; width:30%;"><?php esc_html_e( 'WP Pugmill', 'wp-pugmill' ); ?></th>
			</tr>
		</thead>
		<tbody>
		<?php
		$seo_plugin_label = $has_seo_plugin ? esc_html( wppugmill_seo_plugin_names() ) : '<span style="color:#6b7280">' . esc_html__( 'No plugin', 'wp-pugmill' ) . '</span>';
		$plugin_badge     = $has_seo_plugin ? '<span style="background:#f0f6ff; border:1px solid #b3d1f5; border-radius:3px; padding:2px 8px; font-size:11px; font-weight:600; color:#2271b1;">' . esc_html( wppugmill_seo_plugin_names() ) . '</span>' : '<span style="color:#6b7280; font-size:12px;">' . esc_html__( 'None active', 'wp-pugmill' ) . '</span>';

		$overlap_rows = array(
			array(
				'label'       => __( 'Meta Description', 'wp-pugmill' ),
			...
			array(
				...
				'desc'        => __( 'BlogPosting / WebPage schema node', 'wp-pugmill' ),
				'option'      => 'wppugmill_disable_json_ld',
				'current_val' => $disable_json_ld,
			),
			array(
				'label'       => __( 'Breadcrumb Schema', 'wp-pugmill' ),
				'desc'        => __( 'BreadcrumbList JSON-LD', 'wp-pugmill' ),
				'option'      => 'wppugmill_disable_breadcrumbs',
		...
		<div style="margin-top:14px; padding:10px 14px; background:#f0faf0; border:1px solid #c3e6c3; border-radius:4px; font-size:12px; color:#1e6e2e;">
			&#128161; <?php printf(
				esc_html__( 'Recommended: Disable Meta Description, Open Graph Tags, Twitter Cards, Article JSON-LD, and Breadcrumb Schema — let %s handle them. WP Pugmill will still output its unique AEO additions (FAQPage, entity graph, citations) alongside it.', 'wp-pugmill' ),
				esc_html( wppugmill_seo_plugin_names() )
			); ?>
		</div>
		<?php endif; ?>
		...
		<p style="<?php echo esc_attr( $p_style ); ?> margin-top:0;"><?php esc_html_e( 'WP Pugmill generates three key crawl files: an XML sitemap, a robots.txt, and an llms.txt. View live file previews and resolve conflicts if another plugin is already handling one of these files.', 'wp-pugmill' ); ?></p>
		...
		<div class="wppugmill-file-row">
			...
			<div class="wppugmill-preview-card <?php echo ! empty( $compat['sitemap_conflicts'] ) ? 'has-conflict' : ''; ?>">
				<div class="wppugmill-preview-card-header">
					...
					<span style="font-weight:600; font-size:13px; color:#1e6e2e;"><?php esc_html_e( 'No sitemap conflicts', 'wp-pugmill' ); ?></span>
					<?php else : ?>
			...
			<div class="wppugmill-conflict-block">
				<div class="wppugmill-conflict-block-title">
					&#9888; <?php esc_html_e( 'Sitemap conflict — another plugin may be serving /sitemap.xml', 'wp-pugmill' ); ?>
				</div>
		...
		<div class="wppugmill-file-row">
			<h3>/llms.txt</h3>
			<p style="<?php echo esc_attr( $p_style ); ?>"><?php esc_html_e( 'Serves a structured index of your content in the format AI assistants and LLMs expect. Pugmill\'s version includes your AEO metadata — summaries, Q&A pairs, and entity data.', 'wp-pugmill' ); ?></p>
			<p style="margin:0 0 12px;">
				<a href="<?php echo esc_url( home_url( '/llms.txt' ) ); ?>" target="_blank" class="button button-secondary">
					<?php esc_html_e( 'View llms.txt →', 'wp-pugmill' ); ?>
				</a>
			</p>
			...
			<div class="wppugmill-preview-card <?php echo ! empty( $compat['llms_txt_conflicts'] ) ? 'has-conflict' : ''; ?>">
				<div class="wppugmill-preview-card-header">
					...
					<span style="color:#46b450; font-weight:bold;">&#10003;</span>
					<span style="font-weight:600; font-size:13px; color:#1e6e2e;"><?php esc_html_e( 'No llms.txt conflicts', 'wp-pugmill' ); ?></span>
					<?php else : ?>
					<span style="font-weight:600; font-size:13px; color:#333;">
						<?php printf(
			...
			<div class="wppugmill-conflict-block">
				<div class="wppugmill-conflict-block-title">
					&#9888; <?php esc_html_e( 'llms.txt conflict — another plugin may be serving /llms.txt', 'wp-pugmill' ); ?>
				</div>
		...
		<div class="wppugmill-file-row">
			<h3>/robots.txt</h3>
			<p style="<?php echo esc_attr( $p_style ); ?>"><?php esc_html_e( 'Every bot category — AI answer crawlers, training crawlers, search indexers, and SEO tools — checks robots.txt before accessing any content. It is not an SEO-only concern; it is the universal access control layer for all automated web traffic.', 'wp-pugmill' ); ?></p>
			<p style="margin:0 0 12px;">
				<a href="<?php echo esc_url( home_url( '/robots.txt' ) ); ?>" target="_blank" class="button button-secondary">
					<?php esc_html_e( 'View robots.txt →', 'wp-pugmill' ); ?>
				</a>
			</p>

			<div class="wppugmill-preview-card">
				<div class="wppugmill-preview-card-header">
			...
			<div style="margin-top:16px;">
				<p style="<?php echo esc_attr( $p_style ); ?> margin:0 0 6px;"><?php esc_html_e( 'Override WordPress\'s virtual robots.txt with your own content. WP Pugmill appends a Sitemap: directive automatically. Leave blank to use WordPress defaults.', 'wp-pugmill' ); ?></p>
				<textarea
		...
		<div style="background:#faf7ff; border:1px solid #d4c8f0; border-radius:8px; padding:20px 24px; margin:24px 0 0;">
			<div style="display:flex; justify-content:space-between; align-items:center; gap:16px;">
				...
				<?php if ( get_option( 'wppugmill_analytics_opted_in' ) ) : ?>
				<button id="wppugmill-send-now" type="button"
					...
					<?php esc_html_e( 'Send to Network', 'wp-pugmill' ); ?>
				</button>
		...
		if ( $donut_top_rest > 0 ) {
			$donut_topbots[] = array( 'label' => __( 'Others', 'wp-pugmill' ), 'value' => $donut_top_rest, 'color' => '#d1d5db' );
		}
