 ...
 * and appends them to the daily intelligence payload sent to pugmill.dev.
 *
 ...
 *   industry          value of wppugmill_industry option (admin-set)
 *
 * @package WPPugmill
 */
...
define( 'WPPUGMILL_SIGNAL_DB_VERSION', '2' );
 ...
 * @return string|false
 */
function wppugmill_intel_current_bot( $set = null ) {
	static $bot = false;
	if ( null !== $set ) {
		$bot = $set;
	}
 ...
 */
function wppugmill_signal_install() {
	global $wpdb;
	...
	if ( '1' === get_option( 'wppugmill_signal_db_version' ) ) {
		$wpdb->query( "DROP TABLE IF EXISTS {$wpdb->prefix}wppugmill_signal_daily" ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
	}

	$cc  = $wpdb->get_charset_collate();
	$sql = "CREATE TABLE {$wpdb->prefix}wppugmill_signal_daily (
	...
	dbDelta( $sql );
	update_option( 'wppugmill_signal_db_version', WPPUGMILL_SIGNAL_DB_VERSION );
}

/**
 * Lazy-install: run schema creation on plugins_loaded if not up to date.
 */
function wppugmill_signal_maybe_install() {
	if ( get_option( 'wppugmill_signal_db_version' ) !== WPPUGMILL_SIGNAL_DB_VERSION ) {
		wppugmill_signal_install();
	}
}
add_action( 'plugins_loaded', 'wppugmill_signal_maybe_install' );
 ...
 * @param string $bucket
 */
function wppugmill_intel_tally( $day, $bot, $metric, $bucket ) {
	global $wpdb;
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	$wpdb->query( $wpdb->prepare(
		"INSERT INTO {$wpdb->prefix}wppugmill_signal_daily (day, bot, metric, bucket, tally)
		 VALUES (%d, %s, %s, %s, 1)
 ...
 * @param int    $value
 */
function wppugmill_intel_tally_add( $day, $bot, $metric, $bucket, $value ) {
	global $wpdb;
	$v = (int) $value;
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	$wpdb->query( $wpdb->prepare(
...
function wppugmill_intel_init() {
	...
	wppugmill_intel_current_bot( $bot );

	// PHP generation time — measured from WPPUGMILL_REQUEST_START (defined in
	// wp-pugmill.php at plugin bootstrap) to PHP process shutdown.
	register_shutdown_function( 'wppugmill_intel_shutdown' );

	// Rich per-request signals — WP query functions are reliable at template_redirect.
	add_action( 'template_redirect', 'wppugmill_intel_capture', 10 );
}
add_action( 'init', 'wppugmill_intel_init', 100 );
 ...
 */
function wppugmill_intel_capture() {
	$bot = wppugmill_intel_current_bot();
	if ( ! $bot ) {
		return;
	}
	...
	// ── 404 ───────────────────────────────────────────────────────────────
	if ( is_404() ) {
		wppugmill_intel_tally( $day, $bot, 'http_status',   '404' );
		wppugmill_intel_tally( $day, $bot, 'request_type',  '404' );
		return;
	}

	// ── Feed (RSS / Atom / JSON feed) ─────────────────────────────────────
	if ( is_feed() ) {
		wppugmill_intel_tally( $day, $bot, 'http_status',  '200' );
		wppugmill_intel_tally( $day, $bot, 'request_type', 'feed' );
		return;
	}
	...
	if ( is_search() ) {
		wppugmill_intel_tally( $day, $bot, 'http_status',  '200' );
		wppugmill_intel_tally( $day, $bot, 'request_type', 'search' );
		wppugmill_intel_tally( $day, $bot, 'bot_searched', 'true' );
		...
		wppugmill_intel_tally( $day, $bot, 'search_query_words', $q_bucket );
		return;
	}

	// ── Standard HTML request ─────────────────────────────────────────────
	wppugmill_intel_tally( $day, $bot, 'http_status',  '200' );
	wppugmill_intel_tally( $day, $bot, 'request_type', 'html' );

	// URL depth (path segment count, capped at 9 to bound the bucket space).
	$path  = (string) parse_url( $uri, PHP_URL_PATH );
	$depth = max( 0, substr_count( rtrim( $path, '/' ), '/' ) );
	wppugmill_intel_tally( $day, $bot, 'url_depth', (string) min( $depth, 9 ) );
	...
	if ( $qs ) {
		...
		$wp_reserved = array_flip( array(
			...
			'page', 'preview', 'm', 'wppugmill_llm',
		) );
		$url_type = empty( array_diff_key( $params, $wp_reserved ) ) ? 'clean' : 'parameterized';
	} else {
		$url_type = 'clean';
	}
	wppugmill_intel_tally( $day, $bot, 'url_type', $url_type );
	...
	if ( ! is_singular() ) {
		...
		wppugmill_intel_tally( $day, $bot, 'content_type', $content_type );
		return;
	}
	...
	wppugmill_intel_tally( $day, $bot, 'content_type', $ct_bucket );
	...
	wppugmill_intel_tally( $day, $bot, 'content_freshness', $freshness );
	...
	wppugmill_intel_tally( $day, $bot, 'word_count', $wc_bucket );
	...
	if ( $total > 0 ) {
		...
		wppugmill_intel_tally( $day, $bot, 'fact_density', $fd );
	}
}
 ...
 */
function wppugmill_intel_shutdown() {
	if ( ! defined( 'WPPUGMILL_REQUEST_START' ) ) {
		return;
	}
	if ( ! wppugmill_intel_current_bot() ) {
		return;
	}

	$gen_ms = (int) round( ( microtime( true ) - WPPUGMILL_REQUEST_START ) * 1000 );
	...
	$bot = wppugmill_intel_current_bot();
	$day = (int) floor( time() / DAY_IN_SECONDS );
	wppugmill_intel_tally_add( $day, $bot, 'php_gen_ms_sum',   'all', $gen_ms );
	wppugmill_intel_tally(     $day, $bot, 'php_gen_ms_count', 'all' );
}
 ...
 */
function wppugmill_intel_build_signals( $day ) {
	global $wpdb;

	// phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	$rows = $wpdb->get_results( $wpdb->prepare(
		"SELECT bot, metric, bucket, tally
		 FROM {$wpdb->prefix}wppugmill_signal_daily
		 WHERE day = %d",
		$day
	), ARRAY_A );
	...
	$site_meta = wppugmill_intel_site_meta();
 ...
 */
function wppugmill_intel_get_signals_30d( $days = 30 ) {
	global $wpdb;
 ...
 */
function wppugmill_intel_site_meta() {
	$cached = get_transient( 'wppugmill_intel_site_meta' );
	if ( false !== $cached ) {
		return $cached;
	}
	...
	$meta = array(
		'size_tier' => $size_tier,
		'industry'  => sanitize_text_field( (string) get_option( 'wppugmill_industry', '' ) ),
	);

	set_transient( 'wppugmill_intel_site_meta', $meta, DAY_IN_SECONDS );
	return $meta;
}
 ...
 *
 * Hooked via the 'wppugmill_intelligence_payload' filter added in
 ...
 * @return array
 */
function wppugmill_intel_filter_payload( $payload, $day ) {
	$signals = wppugmill_intel_build_signals( $day );
	if ( ! empty( $signals ) ) {
		$payload['signals'] = $signals;
	}
	return $payload;
}
add_filter( 'wppugmill_intelligence_payload', 'wppugmill_intel_filter_payload', 10, 2 );

// ── Pruning ────────────────────────────────────────────────────────────────────

/**
 * Prune signal_daily rows older than 90 days.
 * Piggybacked on the existing wppugmill_daily_prune cron event.
 */
function wppugmill_intel_prune() {
	global $wpdb;
	$cutoff = (int) floor( ( time() - ( 90 * DAY_IN_SECONDS ) ) / DAY_IN_SECONDS );
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	$wpdb->query( $wpdb->prepare(
		"DELETE FROM {$wpdb->prefix}wppugmill_signal_daily WHERE day < %d",
		$cutoff
	) );
}
add_action( 'wppugmill_daily_prune', 'wppugmill_intel_prune' );
