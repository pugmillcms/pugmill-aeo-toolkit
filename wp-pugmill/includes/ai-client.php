<?php
/**
 * WP Pugmill — AI transport layer.
 *
 * Everything that touches external AI providers or orchestrates a request:
 *   - wppugmill_ai_request_setup()   — nonce/cap/rate-limit/key preamble
 *   - wppugmill_call_ai()            — generic provider dispatcher
 *   - wppugmill_ai_request_args()    — shared HTTP args (timeout, sslverify)
 *   - wppugmill_record_token_usage() — persist per-user token counts
 *   - wppugmill_get_token_usage()    — read per-user token counts
 *
 * No AJAX action registrations live here — this file is pure infrastructure.
 * Add a new provider by extending the switch in wppugmill_call_ai().
 *
 * @package WPPugmill
 */
	...
	if ( $require_license ) {
		...
		if ( 'free' === $mode ) {
			wp_send_json_error( array(
				...
				'message' => sprintf( __( '%1$s requires WP Pugmill Pro. <a href="https://wppugmill.com/pricing" target="_blank">Get your license →</a>', 'wp-pugmill' ), $feature_label ),
			), 403 );
		}
		if ( 'pro' === $mode ) {
...
function wppugmill_call_ai( $provider, $api_key, $system, $user, $max_tokens = 400 ) {
	...
	if ( 200 !== $code ) {
		...
		if ( 529 === $code ) return new WP_Error( 'provider_error', __( 'AI provider is momentarily overloaded. Please try again in a few seconds.', 'wp-pugmill' ) );
 ...
 * @return string|WP_Error
 */
function wppugmill_call_ai_vision( $provider, $api_key, $image_url, $prompt, $max_tokens = 100 ) {
	switch ( $provider ) {
		case 'openai':
			$response = wp_remote_post(
				'https://api.openai.com/v1/chat/completions',
		...
		case 'gemini':
			...
			if ( is_wp_error( $img_response ) ) {
				return new WP_Error( 'image_fetch_failed', __( 'Could not fetch the featured image for analysis.', 'wp-pugmill' ) );
			}
		...
		default:
			...
			if ( str_starts_with( $image_url, 'https://' ) ) {
				...
				if ( is_wp_error( $img_response ) ) {
					return new WP_Error( 'image_fetch_failed', __( 'Could not fetch the image for analysis.', 'wp-pugmill' ) );
				}
				$img_body   = wp_remote_retrieve_body( $img_response );
				$mime_type  = wp_remote_retrieve_header( $img_response, 'content-type' ) ?: 'image/jpeg';
	...
	if ( is_wp_error( $response ) ) {
		error_log( 'WP Pugmill vision error (' . $provider . '): ' . $response->get_error_message() );
		return new WP_Error( 'request_failed', __( 'Could not reach AI provider. Please check your connection and try again.', 'wp-pugmill' ) );
	}
	...
	if ( 200 !== $code ) {
		$detail = $data['error']['message'] ?? $body;
		error_log( 'WP Pugmill vision provider error (' . $provider . ' ' . $code . '): ' . $detail );
		if ( 401 === $code ) return new WP_Error( 'provider_error', __( 'Invalid API key.', 'wp-pugmill' ) );
		if ( 429 === $code ) return new WP_Error( 'provider_error', __( 'AI provider rate limit reached. Please wait and try again.', 'wp-pugmill' ) );
		return new WP_Error( 'provider_error', __( 'AI provider returned an error. Please try again.', 'wp-pugmill' ) );
	}
 ...
 */
function wppugmill_record_token_usage( $provider, $data ) {
	...
	$meta_key = 'wppugmill_tokens_' . gmdate( 'Y_m' );
 ...
 */
function wppugmill_get_token_usage( $month = '' ) {
	if ( ! $month ) {
		$month = gmdate( 'Y_m' );
	}
	$data = get_user_meta( get_current_user_id(), 'wppugmill_tokens_' . $month, true );
	if ( ! is_array( $data ) ) {
