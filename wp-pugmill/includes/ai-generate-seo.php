<?php
/**
 * WP Pugmill — SEO and schema generation AJAX handlers.
 *
 * Covers:
 *   - wppugmill_ajax_generate_seo()         — SEO title + meta description
 *   - wppugmill_ajax_generate_howto_steps() — HowTo schema steps
 *   - wppugmill_ajax_suggest_schema()        — detect Schema.org type + pre-fill fields
 *
...
function wppugmill_ajax_generate_seo() {
	$r = wppugmill_ai_request_setup( 'wppugmill_generate_seo', 'SEO Generator', false );
...
Return ONLY a JSON object with key "type" and, when a type is chosen, a second key matching the lowercase type name (e.g. "howto", "product"). Populate only fields you can infer from the content — leave others as empty strings. No markdown, no explanation.';
	...
	// Validate type
	$decoded = wppugmill_decode_ai_json( $result, $provider );
	if ( is_wp_error( $decoded ) ) {
		wp_send_json_error( array( 'message' => $decoded->get_error_message() ), 500 );
	}
