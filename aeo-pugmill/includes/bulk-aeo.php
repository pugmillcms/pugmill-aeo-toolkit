 ...
 *
 * Two AJAX endpoints:
 *   aeopugmill_bulk_aeo_get_queue  — return eligible post IDs + coverage stats
 *   aeopugmill_bulk_aeo_process    — generate + save AEO for one post (Pro only)
 *
 ...
 *
 * Saves directly to post meta via aeopugmill_save_aeo() — no editor state involved.
 *
 * @package WPPugmill
 */
...
add_action( 'wp_ajax_aeopugmill_bulk_aeo_get_queue', 'aeopugmill_ajax_bulk_aeo_get_queue' );
add_action( 'wp_ajax_aeopugmill_bulk_aeo_process',   'aeopugmill_ajax_bulk_aeo_process' );
 ...
 */
function aeopugmill_ajax_bulk_aeo_get_queue() {
	check_ajax_referer( 'aeopugmill_bulk_aeo', 'nonce' );
		...
		$wpdb->prepare(
			...
			"SELECT p.ID, pm.meta_value AS aeo_meta
			 ...
			 LEFT JOIN {$wpdb->postmeta} pm
			   ON pm.post_id = p.ID AND pm.meta_key = '_aeopugmill_aeo'
	...
	foreach ( (array) $rows as $row ) {
		...
		if ( function_exists( 'aeopugmill_post_is_noindexed' ) && aeopugmill_post_is_noindexed( $post_id ) ) {
			continue;
		}
 ...
 */
function aeopugmill_ajax_bulk_aeo_process() {
	check_ajax_referer( 'aeopugmill_bulk_aeo', 'nonce' );
	...
	if ( 'ai' !== aeopugmill_mode() ) {
		wp_send_json_error( array( 'message' => __( 'AEO Pugmill Pro required.', 'aeo-pugmill' ) ), 403 );
	}

	$post_id = absint( wp_unslash( $_POST['post_id'] ?? 0 ) );
	if ( ! $post_id ) {
		wp_send_json_error( array( 'message' => __( 'Invalid post ID.', 'aeo-pugmill' ) ), 400 );
	}

	$post = get_post( $post_id );
	if ( ! $post || 'publish' !== $post->post_status ) {
		wp_send_json_error( array( 'message' => __( 'Post not found or not published.', 'aeo-pugmill' ) ), 404 );
	}
	...
	if ( empty( $api_key ) ) {
		wp_send_json_error( array( 'message' => __( 'No AI API key configured. Add one under Settings → AI Provider.', 'aeo-pugmill' ) ), 400 );
	}
	...
	$raw = aeopugmill_call_ai(
		...
		aeopugmill_aeo_user_prompt( $title, $content ),
		2048
	);
	...
	$aeo = aeopugmill_decode_ai_json( $raw, $provider );
	if ( is_wp_error( $aeo ) ) {
		...
		'entities'  => array_values( array_filter(
			array_map(
				function( $entity ) use ( $allowed_types ) {
					...
					if ( function_exists( 'aeopugmill_validate_same_as_url' ) ) {
	...
	aeopugmill_save_aeo( $post_id, $clean );
