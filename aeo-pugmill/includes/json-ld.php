 ...
 *           using the cascade: _aeopugmill_seo fields → AEO summary → excerpt/bloginfo
 *
 * Home / front page:
 ...
 *
 * Note: aeopugmill_disable_json_ld suppresses the Article/BlogPosting and
 ...
 */
function aeopugmill_output_json_ld() {
	if ( is_singular() ) {
		aeopugmill_output_singular_json_ld();
	}

	// Home/front-page: suppress entire block when deferring JSON-LD to SEO plugin.
	if ( ( is_home() || is_front_page() ) && ! get_option( 'aeopugmill_disable_json_ld' ) ) {
		aeopugmill_output_home_json_ld();
	}
}
add_action( 'wp_head', 'aeopugmill_output_json_ld' );
 ...
 *
 *   aeopugmill_disable_json_ld   — suppress Article/BlogPosting node, extended schema,
 *                                  and BreadcrumbList (defer to active SEO plugin).
 *                                  FAQPage is still output — it is AEO-exclusive.
 *
 *   aeopugmill_disable_breadcrumbs — suppress only BreadcrumbList (independent control).
 *                                    Implied when aeopugmill_disable_json_ld is also set.
 *
 * Nodes: Article/WebPage (conditional), FAQPage (always if Q&A exists),
 *        extended schema (conditional), BreadcrumbList (conditional).
 */
function aeopugmill_output_singular_json_ld() {
	$post_id = get_the_ID();
	$post    = get_post( $post_id );
	$aeo     = aeopugmill_get_aeo( $post_id );
	$seo     = aeopugmill_get_seo( $post_id );

	$defer_to_seo_plugin = (bool) get_option( 'aeopugmill_disable_json_ld', 0 );
	$suppress_breadcrumbs = $defer_to_seo_plugin || (bool) get_option( 'aeopugmill_disable_breadcrumbs', 0 );
	...
	if ( ! $defer_to_seo_plugin ) {
		$graph[] = aeopugmill_build_article_node( $post_id, $post, $aeo, $seo );
	}
	...
	$faq = aeopugmill_build_faq_node( get_permalink( $post_id ), $aeo['questions'] );
	if ( $faq ) {
		...
		if ( ! empty( $aeo['summary'] ) ) {
			$faq['associatedMedia'] = array(
				...
				'contentUrl'     => add_query_arg( 'aeopugmill_llm', '1', get_permalink( $post_id ) ),
	...
	if ( ! $defer_to_seo_plugin ) {
		$extended = aeopugmill_build_extended_schema_node( $post_id, $post );
		if ( $extended ) {
			$graph[] = $extended;
		}
	}
	...
	if ( ! $suppress_breadcrumbs ) {
		$breadcrumbs = aeopugmill_build_breadcrumb_schema( $post_id );
 ...
 * @return array
 */
function aeopugmill_build_article_node( $post_id, $post, $aeo, $seo ) {
	...
	$description = aeopugmill_resolve_description( $seo, $aeo, $post );
	if ( $description ) {
	...
	$org_name  = get_option( 'aeopugmill_org_name', '' ) ?: get_bloginfo( 'name' );
	$publisher = array(
		'@type' => get_option( 'aeopugmill_org_type', '' ) ?: 'Organization',
		'name'  => $org_name,
	);
	$logo_url = aeopugmill_get_site_image_url();
	if ( $logo_url ) {
	...
	if ( ! $is_page && $post->post_author ) {
		...
		$same_as_raw = get_option( 'aeopugmill_author_same_as', '' );
		if ( $same_as_raw ) {
	...
	// node in aeopugmill_output_singular_json_ld() so it survives Yoast coexistence
 ...
 * @return array|null
 */
function aeopugmill_build_faq_node( $permalink, $questions ) {
 ...
 */
function aeopugmill_output_home_json_ld() {
	$org_name    = get_option( 'aeopugmill_org_name', '' ) ?: get_bloginfo( 'name' );
	$site_url    = home_url( '/' );
	...
	if ( get_option( 'aeopugmill_org_name', '' ) ) {
		$org = array(
			'@type' => get_option( 'aeopugmill_org_type', '' ) ?: 'Organization',
			'name'  => $org_name,
			'url'   => $site_url,
 ...
 */
function aeopugmill_build_breadcrumb_schema( $post_id ) {
	$items    = array();
	$position = 1;
 ...
 *
 * Suppressed entirely when aeopugmill_disable_seo_meta is set — this defers
 ...
 */
function aeopugmill_output_meta_tags() {
	if ( get_option( 'aeopugmill_disable_seo_meta' ) ) {
		return;
	}

	if ( is_singular() ) {
		aeopugmill_output_singular_meta_tags();
		return;
	}

	if ( is_home() || is_front_page() ) {
		aeopugmill_output_home_meta_tags();
	}
}
add_action( 'wp_head', 'aeopugmill_output_meta_tags', 2 );

/**
 * Meta tags for singular post/page views.
 *
 * Cascades: _aeopugmill_seo fields → AEO summary → post excerpt/title
 */
function aeopugmill_output_singular_meta_tags() {
	$post_id = get_the_ID();
	$post    = get_post( $post_id );
	$aeo     = aeopugmill_get_aeo( $post_id );
	$seo     = aeopugmill_get_seo( $post_id );
 ...
 */
function aeopugmill_output_home_meta_tags() {
	$org_name    = get_option( 'aeopugmill_org_name', '' );
	$title       = $org_name ?: get_bloginfo( 'name' );
	$description = get_option( 'aeopugmill_site_summary', get_bloginfo( 'description' ) );
	$url         = home_url( '/' );
	$image_url   = aeopugmill_get_site_image_url();
 ...
 * @return array
 */
function aeopugmill_get_schema( $post_id ) {
	$defaults = array(
		'type'           => 'Article',
	...
	$raw = get_post_meta( (int) $post_id, '_aeopugmill_schema', true );
	if ( empty( $raw ) ) {
		return $defaults;
	}
 ...
 * @return array|null
 */
function aeopugmill_build_extended_schema_node( $post_id, $post ) {
	$schema = aeopugmill_get_schema( $post_id );
	$type   = $schema['type'] ?? '';
	...
	switch ( $type ) {
		...
		case 'HowTo':
			return aeopugmill_build_howto_node( $post_id, $post, $schema['howto'] );
		case 'Product':
			return aeopugmill_build_product_node( $post_id, $post, $schema['product'] );
		case 'Event':
			return aeopugmill_build_event_node( $post_id, $post, $schema['event'] );
		case 'LocalBusiness':
			return aeopugmill_build_local_business_node( $post_id, $post, $schema['local_business'] );
		case 'VideoObject':
			return aeopugmill_build_video_node( $post_id, $post, $schema['video'] );
		case 'Review':
			return aeopugmill_build_review_node( $post_id, $post, $schema['review'] );
		default:
			return null;
	}
}
 ...
 * HowTo schema node.
 */
function aeopugmill_build_howto_node( $post_id, $post, $data ) {
	$node = array(
		'@type' => 'HowTo',
		'name'  => get_the_title( $post_id ),
	);
 ...
 * Product schema node.
 */
function aeopugmill_build_product_node( $post_id, $post, $data ) {
	$node = array(
		'@type' => 'Product',
 ...
 * Event schema node.
 */
function aeopugmill_build_event_node( $post_id, $post, $data ) {
	$node = array(
		'@type'     => 'Event',
		'name'      => ! empty( $data['name'] ) ? $data['name'] : get_the_title( $post_id ),
 ...
 */
function aeopugmill_build_local_business_node( $post_id, $post, $data ) {
	$org_name = get_option( 'aeopugmill_org_name', '' ) ?: get_bloginfo( 'name' );
	$node = array(
	...
	$logo = aeopugmill_get_site_image_url();
	if ( $logo ) {
		$node['logo'] = $logo;
	}
 ...
 */
function aeopugmill_build_video_node( $post_id, $post, $data ) {
	$node = array(
		'@type' => 'VideoObject',
		'name'  => ! empty( $data['name'] ) ? $data['name'] : get_the_title( $post_id ),
	);
 ...
 * Review schema node.
 */
function aeopugmill_build_review_node( $post_id, $post, $data ) {
	$item_type_map = array(
		'Book'                => 'Book',
		'Movie'               => 'Movie',
		'Product'             => 'Product',
 ...
 *
 * @param  array   $seo  _aeopugmill_seo data.
 * @param  array   $aeo  _aeopugmill_aeo data.
 * @param  WP_Post $post
 * @return string
 */
function aeopugmill_resolve_description( $seo, $aeo, $post ) {
 ...
 */
function aeopugmill_get_site_image_url() {
