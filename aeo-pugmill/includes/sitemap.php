 ...
 * @return bool
 */
function aeopugmill_own_noindex( $post_id ) {
	$raw = get_post_meta( $post_id, '_aeopugmill_seo', true );
	if ( ! $raw ) {
		return false;
	}
 ...
 */
function aeopugmill_sitemap_rewrite() {
	add_rewrite_rule( '^sitemap\.xml$', 'index.php?aeopugmill_sitemap=1', 'top' );
	add_rewrite_tag( '%aeopugmill_sitemap%', '([0-9]+)' );
}
add_action( 'init', 'aeopugmill_sitemap_rewrite' );
 ...
 */
function aeopugmill_maybe_serve_sitemap() {
	if ( intval( get_query_var( 'aeopugmill_sitemap' ) ) !== 1 ) {
		return;
	}

	// User has chosen to let another plugin handle the sitemap — step aside.
	if ( get_option( 'aeopugmill_disable_sitemap' ) ) {
		return;
	}
	...
	foreach ( aeopugmill_sitemap_collect_urls() as $entry ) {
		echo "\t<url>\n";
...
add_action( 'template_redirect', 'aeopugmill_maybe_serve_sitemap', 1 );
 ...
 */
function aeopugmill_sitemap_collect_urls() {
	$urls = array();

	// Home page
	$urls[] = array(
		'loc'        => home_url( '/' ),
		'lastmod'    => aeopugmill_sitemap_date( get_lastpostmodified( 'GMT' ) ),
		'changefreq' => 'daily',
		'priority'   => '1.0',
	);
	...
	foreach ( $post_types as $post_type ) {
		...
		foreach ( $posts as $post ) {
			...
			if ( aeopugmill_own_noindex( $post->ID ) || aeopugmill_post_is_noindexed( $post->ID ) ) {
				continue;
			}
			...
			$urls[] = array(
				'loc'        => $permalink,
				'lastmod'    => aeopugmill_sitemap_date( $post->post_modified_gmt ),
				'changefreq' => 'weekly',
				'priority'   => $is_front ? '1.0' : ( 'post' === $post_type ? '0.8' : '0.6' ),
				'markdown'   => add_query_arg( 'aeopugmill_llm', '1', $permalink ),
			);
		}
	}
	 ...
	 */
	return apply_filters( 'aeopugmill/sitemap/urls', $urls );
}
 ...
 * @return string
 */
function aeopugmill_sitemap_date( $date ) {
	$ts = strtotime( $date );
 ...
 * @param bool    $update
 */
function aeopugmill_ping_search_engines( $post_id, $post, $update ) {
	...
	$throttle_key = 'aeopugmill_ping_throttle';
	...
	$indexnow_key = aeopugmill_indexnow_key();
	wp_remote_get(
		add_query_arg(
			array(
...
add_action( 'save_post', 'aeopugmill_ping_search_engines', 10, 3 );
 ...
 * @return string
 */
function aeopugmill_indexnow_key() {
	$key = get_option( 'aeopugmill_indexnow_key', '' );
	if ( '' === $key ) {
		$key = bin2hex( random_bytes( 16 ) ); // 32 hex chars
		update_option( 'aeopugmill_indexnow_key', $key, false );
	}
	return $key;
}
 ...
 */
function aeopugmill_indexnow_rewrite() {
	$key = aeopugmill_indexnow_key();
	add_rewrite_rule( '^' . preg_quote( $key, '/' ) . '\.txt$', 'index.php?aeopugmill_indexnow=1', 'top' );
	add_rewrite_tag( '%aeopugmill_indexnow%', '([0-9]+)' );
}
add_action( 'init', 'aeopugmill_indexnow_rewrite' );

/**
 * Serve the IndexNow key file when requested.
 */
function aeopugmill_serve_indexnow_key() {
	if ( intval( get_query_var( 'aeopugmill_indexnow' ) ) !== 1 ) {
		return;
	}
	remove_filter( 'template_redirect', 'redirect_canonical' );
	header( 'Content-Type: text/plain; charset=UTF-8' );
	echo esc_html( aeopugmill_indexnow_key() );
	exit;
}
add_action( 'template_redirect', 'aeopugmill_serve_indexnow_key', 1 );
 ...
 * @return string
 */
function aeopugmill_filter_robots_txt( $output, $public ) {
	$custom = trim( get_option( 'aeopugmill_robots_txt_custom', '' ) );
	...
	if ( $public ) {
		...
		// will discover llms.txt and from there the per-post ?aeopugmill_llm=1 endpoints.
...
add_filter( 'robots_txt', 'aeopugmill_filter_robots_txt', 10, 2 );
