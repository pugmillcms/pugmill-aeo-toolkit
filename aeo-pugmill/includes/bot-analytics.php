...
function aeopugmill_resource_type_categories() {
	return array(
		...
		7  => 'crawl',      // HTML+AEO — a crawl of an AEO-enriched post, grouped with plain HTML
		8  => 'aeo',        // AEO JSON-LD
		9  => 'discovery',  // RSS Feed — how aggregators discover new content
...
## Bot Activity
Which bots are most active and what that signals (citation activity, indexing depth, content discovery phase). Inside content_reach, treat these resource types as strong positive AEO signals because they mean a bot is reading optimized content directly: llms.txt, llms-full.txt, Post Markdown, Site Summary, and AEO JSON-LD. Treat the AEO Post type (HTML page that actually has AEO metadata embedded) as a secondary positive signal — the bot is crawling HTML but landing on an AEO-enriched page. Plain HTML Page without AEO is neutral crawl traffic. RSS Feed and Well-Known hits (robots.txt, .well-known/*, ads.txt, etc.) are discovery signals — they tell you a bot is probing for feeds or metadata, not reading content. Mention the AEO conversion percentage (share of visits that hit AEO endpoints — llms.txt, llms-full.txt, Post Markdown, Site Summary, AEO JSON-LD — vs generic crawling) and whether it is healthy.
...
## Content Coverage
Which resource types in content_reach are hit most, and any patterns worth noting. Compare the mix: heavy HTML Page hits with little AEO Post activity means bots are landing on non-AEO content — an optimization gap. Heavy AEO Post or AEO endpoint hits means bots are reaching the enriched content. Call out ignored sections, repeat visits on specific posts, or bots that only touch discovery signals (RSS Feed, Well-Known) without reading actual content.
