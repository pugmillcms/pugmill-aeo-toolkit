<?php
/**
 * AEO Pugmill — Content focus and optimization AI AJAX handlers.
 *
 * Covers:
 *   - aeopugmill_ajax_topic_focus()          — Topic Focus analysis
 *   - aeopugmill_ajax_swap_focus_passage()   — AI passage rewrite for focus fixes
 *   - aeopugmill_ajax_refine_focus()         — Identify off-topic passages
 *   - aeopugmill_ajax_fix_keyword_coverage() — Keyword coverage gap fix
 *   - aeopugmill_ajax_suggest_headings()     — Heading insertion suggestions
 *
 * Depends on: ai-client.php (aeopugmill_ai_request_setup, aeopugmill_call_ai),
 *             ai-utils.php (aeopugmill_decode_ai_json, aeopugmill_get_paragraph_block_texts,
 *                           aeopugmill_remap_passage_to_raw),
...
add_action( 'wp_ajax_aeopugmill_topic_focus', 'aeopugmill_ajax_topic_focus' );

function aeopugmill_ajax_topic_focus() {
	$r = aeopugmill_ai_request_setup( 'aeopugmill_topic_focus', 'Topic Focus' );
...
add_action( 'wp_ajax_aeopugmill_swap_focus_passage', 'aeopugmill_ajax_swap_focus_passage' );
...
function aeopugmill_ajax_swap_focus_passage() {
	...
	if ( empty( $passage ) || empty( $recommendation ) ) {
		wp_send_json_error( array( 'message' => __( 'Missing passage or recommendation.', 'aeo-pugmill' ) ), 400 );
	}
	...
	$result = aeopugmill_call_ai( $r['provider'], $r['api_key'], $system, $user, 300 );
...
add_action( 'wp_ajax_aeopugmill_refine_focus', 'aeopugmill_ajax_refine_focus' );

function aeopugmill_ajax_refine_focus() {
	$r = aeopugmill_ai_request_setup( 'aeopugmill_refine_focus', 'Refine Focus' );
	if ( empty( trim( $r['content'] ) ) ) {
		wp_send_json_error( array( 'message' => __( 'Post has no content to analyze.', 'aeo-pugmill' ) ), 400 );
	}
	...
	$raw_content = mb_substr( trim( wp_strip_all_tags( $r['post']->post_content ) ), 0, AEOPUGMILL_MAX_AI_INPUT );
	$para_texts  = aeopugmill_get_paragraph_block_texts( $r['post']->post_content );
...
Return ONLY a JSON object: {"issues":[{"label":"Short label","passage":"Exact single-paragraph quote","recommendation":"What to do about it"}]}. Be specific and actionable. No explanation outside the JSON.';
	...
	// aeopugmill_decode_ai_json may unwrap single-key JSON {"issues":[...]} → [...]
	...
	foreach ( $issues as $issue ) {
		...
		foreach ( $para_texts as $block_text ) {
			$remapped = aeopugmill_remap_passage_to_raw( $passage, $block_text );
	...
	if ( empty( $validated ) ) {
		wp_send_json_error( array( 'message' => __( 'AI suggestions could not be matched to post content. Please try again.', 'aeo-pugmill' ) ), 500 );
	}

	wp_send_json_success( array( 'issues' => $validated ) );
}

add_action( 'wp_ajax_aeopugmill_fix_keyword_coverage', 'aeopugmill_ajax_fix_keyword_coverage' );
 ...
 */
function aeopugmill_ajax_fix_keyword_coverage() {
	$r = aeopugmill_ai_request_setup( 'aeopugmill_fix_keyword_coverage', 'Keyword Coverage Fix' );

	if ( empty( trim( $r['content'] ) ) ) {
		wp_send_json_error( array( 'message' => __( 'Post has no content to analyze.', 'aeo-pugmill' ) ), 400 );
	}

	$aeo      = aeopugmill_get_aeo( $r['post']->ID );
	$keywords = array_filter( $aeo['keywords'], function( $k ) { return ! empty( trim( $k ) ); } );

	if ( empty( $keywords ) ) {
		wp_send_json_error( array( 'message' => __( 'No keywords found in the AEO panel. Add keywords first, then run this fix.', 'aeo-pugmill' ) ), 400 );
	}
...
No markdown fences, no explanation outside the JSON.';
	...
	$result = aeopugmill_call_ai( $r['provider'], $r['api_key'], $system, $user, 1024 );
	...
	foreach ( $raw_suggs as $sugg ) {
		...
		foreach ( $para_texts as $block_text ) {
			$remapped = aeopugmill_remap_passage_to_raw( $quote, $block_text );
			if ( strpos( $block_text, $remapped ) !== false ) {
				$validated[] = array(
					'quote'      => $remapped,
...
add_action( 'wp_ajax_aeopugmill_suggest_headings', 'aeopugmill_ajax_suggest_headings' );
 ...
 */
function aeopugmill_ajax_suggest_headings() {
	$r = aeopugmill_ai_request_setup( 'aeopugmill_suggest_headings', 'Heading Suggestions' );
	...
	if ( empty( $para_texts ) ) {
		wp_send_json_error( array( 'message' => __( 'No paragraph blocks found. Heading insertion requires the block editor.', 'aeo-pugmill' ) ), 400 );
	}
