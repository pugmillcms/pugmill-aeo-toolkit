...
function aeopugmill_post_is_noindexed( $post_id ) {
	...
	// Pugmill AEO Toolkit's own per-post noindex flag
...
// -------------------------------------------------------------------------
// /llms.txt  (and all plain-text output functions below)
// -------------------------------------------------------------------------
// phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- All functions below serve plain-text/Markdown endpoints (Content-Type: text/plain). HTML escaping functions such as esc_html() would corrupt Markdown formatting characters (# > - *). Data is sanitized at write time; these outputs are intentionally unescaped plain text.
...
function aeopugmill_serve_llms_txt() {
	...
	if ( false !== $cached ) {
		echo (string) $cached; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Cache stores pre-escaped plaintext output assembled in this function.
		return;
	}
	...
	if ( $site_summary ) {
		echo esc_html( html_entity_decode( $site_summary, ENT_QUOTES | ENT_HTML5, 'UTF-8' ) ) . "\n\n";
	}
	...
	foreach ( $posts as $post ) {
		...
		$summary = esc_html( html_entity_decode( $summary, ENT_QUOTES | ENT_HTML5, 'UTF-8' ) );
		$url     = esc_url( get_permalink( $post->ID ) );
		$title   = esc_html( html_entity_decode( get_the_title( $post->ID ), ENT_QUOTES | ENT_HTML5, 'UTF-8' ) );
		$llm_url = esc_url( add_query_arg( 'aeopugmill_llm', '1', get_permalink( $post->ID ) ) );
	...
	if ( ! empty( $post_entries ) ) {
		...
		foreach ( $post_entries as $line ) {
			echo $line; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Components escaped at construction (title/url/summary/llm_url).
		}
		echo "\n";
	}
	...
	echo $output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Plaintext output assembled from already-escaped components.
}
...
function aeopugmill_serve_llms_full_txt() {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only pagination param on a public plain-text endpoint; no form submission or state change.
	...
	if ( $page > $total_pages && $total_pages > 0 ) {
		status_header( 404 );
		echo '# Not Found' . "\n\n" . 'Page ' . (int) $page . ' does not exist. Total pages: ' . (int) $total_pages . "\n";
		return;
	}

	ob_start();

	echo '# ' . esc_html( $site_name ) . " — Full Content Index\n\n";
	echo '> ' . esc_url( $site_url ) . "\n\n";
	echo 'Page ' . (int) $page . ' of ' . (int) $total_pages;

	if ( $page < $total_pages ) {
		$next_url = add_query_arg( 'page', $page + 1, home_url( '/llms-full.txt' ) );
		echo ' — Next: ' . esc_url( $next_url );
	}
	...
	foreach ( $query->posts as $post ) {
		...
		echo 'Markdown: ' . esc_url( $llm_url ) . "\n";
		echo 'Published: ' . esc_html( $post->post_date ) . "\n";
		echo 'Modified: ' . esc_html( $post->post_modified ) . "\n\n";

		if ( ! empty( $aeo['summary'] ) ) {
			echo '**Summary:** ' . esc_html( $aeo['summary'] ) . "\n\n";
		}
		...
		if ( ! empty( $aeo['entities'] ) ) {
			...
			foreach ( $aeo['entities'] as $entity ) {
				$desc = ! empty( $entity['description'] ) ? ' — ' . esc_html( $entity['description'] ) : '';
				echo '- ' . esc_html( $entity['name'] ) . ' (' . esc_html( $entity['type'] ) . ')' . $desc . "\n"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $desc pre-escaped above.
			}
			echo "\n";
		}

		if ( ! empty( $aeo['keywords'] ) ) {
			echo '**Keywords:** ' . esc_html( implode( ', ', $aeo['keywords'] ) ) . "\n\n";
		}

		$rendered = apply_filters( 'the_content', $post->post_content ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Applying core WP content filter.
		echo esc_html( aeopugmill_html_to_markdown( $rendered ) ) . "\n\n";
	}
...
function aeopugmill_serve_post_llm_txt() {
	...
	if ( $thumbnail_url ) {
		echo 'Image: ' . esc_url( $thumbnail_url ) . "\n";
	}
	...
	if ( ! empty( $aeo['summary'] ) ) {
		echo "## Summary\n\n";
		echo esc_html( html_entity_decode( $aeo['summary'], ENT_QUOTES | ENT_HTML5, 'UTF-8' ) ) . "\n\n";
	}
	...
	if ( ! empty( $aeo['entities'] ) ) {
		...
		foreach ( $aeo['entities'] as $entity ) {
			$desc = ! empty( $entity['description'] ) ? ' — ' . esc_html( $entity['description'] ) : '';
			$type = ! empty( $entity['type'] ) ? ' (' . esc_html( $entity['type'] ) . ')' : '';
			echo '- ' . esc_html( $entity['name'] ) . $type . $desc . "\n"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $type and $desc pre-escaped above.
		}
		echo "\n";
	}
...
function aeopugmill_serve_site_llm_txt() {
	...
	echo '# ' . esc_html( html_entity_decode( $site_name, ENT_QUOTES | ENT_HTML5, 'UTF-8' ) ) . "\n\n";
	echo 'URL: ' . esc_url( $site_url ) . "\n\n";

	if ( $description ) {
		echo esc_html( html_entity_decode( $description, ENT_QUOTES | ENT_HTML5, 'UTF-8' ) ) . "\n\n";
	}
	...
	echo '- Full index: ' . esc_url( $site_url . 'llms.txt' ) . "\n";
	echo '- Full content: ' . esc_url( $site_url . 'llms-full.txt' ) . "\n\n";
}
// phpcs:enable WordPress.Security.EscapeOutput.OutputNotEscaped

// -------------------------------------------------------------------------
// llms.txt autodiscovery — <link> tag and HTTP Link: header
// -------------------------------------------------------------------------

/**
 * Output a <link> autodiscovery tag for /llms.txt in <head>.
 *
 * Mirrors the pattern RSS autodiscovery uses (<link rel="alternate" type="application/rss+xml">)
 * so AI crawlers that parse <head> for feed/content index hints will find
...
/**
 * Send an HTTP Link: header advertising /llms.txt on every page load.
 *
 * Some crawlers (and HTTP clients) inspect Link: headers before parsing HTML.
 * This gives them a direct signal without requiring an HTML parse.
 */
function aeopugmill_send_llms_link_header() {
	if ( headers_sent() ) {
		return;
	}
	$llms_url      = esc_url_raw( home_url( '/llms.txt' ) );
	$llms_full_url = esc_url_raw( home_url( '/llms-full.txt' ) );
	header( 'Link: <' . $llms_url . '>; rel="llms-index", <' . $llms_full_url . '>; rel="llms-full"', false );
}
add_action( 'send_headers', 'aeopugmill_send_llms_link_header' );
