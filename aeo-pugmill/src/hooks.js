...
/**
 * Read and write the _aeopugmill_aeo post meta field.
 *
...
export function useAeoMeta() {
	...
	const aeo = useMemo( () => {
		const raw = meta && meta._aeopugmill_aeo;
	...
	const updateAeo = useCallback( ( updates ) => {
		...
		setMeta( { ...meta, _aeopugmill_aeo: JSON.stringify( next ) } );
	}, [ aeo, meta, setMeta ] );
...
/**
 * Read and write the _aeopugmill_seo post meta field.
 *
...
export function useSeoMeta() {
	...
	const seo = useMemo( () => {
		const raw = meta && meta._aeopugmill_seo;
	...
	const updateSeo = useCallback( ( updates ) => {
		...
		setMeta( { ...meta, _aeopugmill_seo: JSON.stringify( next ) } );
	}, [ seo, meta, setMeta ] );
...
/**
 * Read and write the _aeopugmill_schema post meta field.
 *
...
export function useSchemaData() {
	...
	const schema = useMemo( () => {
		const raw = meta && meta._aeopugmill_schema;
	...
	const updateSchema = useCallback( ( updates ) => {
		...
		setMeta( { ...meta, _aeopugmill_schema: JSON.stringify( next ) } );
	}, [ schema, meta, setMeta ] );
