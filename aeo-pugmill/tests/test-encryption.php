...
$enc = aeopugmill_encrypt( $key );

assert_true(  'encrypt() returns a non-empty string',       ! empty( $enc ) );
assert_not_equal( 'ciphertext differs from plaintext',      $key, $enc );
assert_equal( 'decrypt(encrypt(key)) recovers original',    $key, aeopugmill_decrypt( $enc ) );

$short = 'abc';
assert_equal( 'short value round-trips correctly', $short, aeopugmill_decrypt( aeopugmill_encrypt( $short ) ) );

$long = str_repeat( 'sk-ant-longkey-', 20 );
assert_equal( 'long value round-trips correctly', $long, aeopugmill_decrypt( aeopugmill_encrypt( $long ) ) );

// ── 2. IV randomness — each encryption is unique ─────────────────────────

section( '2. IV randomness — two encryptions of the same value differ' );

$enc1 = aeopugmill_encrypt( $key );
$enc2 = aeopugmill_encrypt( $key );

assert_not_equal( 'two encryptions of the same value produce different ciphertext', $enc1, $enc2 );
assert_equal( 'both decrypt to the original value (enc1)', $key, aeopugmill_decrypt( $enc1 ) );
assert_equal( 'both decrypt to the original value (enc2)', $key, aeopugmill_decrypt( $enc2 ) );
...
aeopugmill_save_encrypted_option( 'aeopugmill_ai_api_key', $key );

$stored = get_option( 'aeopugmill_ai_api_key', '' );
assert_true(  'stored value is not the plaintext key',  $stored !== $key );
assert_true(  'stored value is a non-empty string',     ! empty( $stored ) );
assert_equal( 'get_encrypted_option recovers original', $key, aeopugmill_get_encrypted_option( 'aeopugmill_ai_api_key' ) );
...
assert_equal( 'missing option returns default',
	'fallback-default',
	aeopugmill_get_encrypted_option( 'aeopugmill_nonexistent_option', 'fallback-default' )
);

aeopugmill_save_encrypted_option( 'aeopugmill_ai_api_key', '' );  // empty string
assert_equal( 'empty plaintext stored as empty, default returned on retrieve',
	'my-default',
	aeopugmill_get_encrypted_option( 'aeopugmill_ai_api_key', 'my-default' )
);
...
$GLOBALS['_wp_options']['aeopugmill_ai_api_key'] = $legacy_key;

$retrieved = aeopugmill_get_encrypted_option( 'aeopugmill_ai_api_key', '' );
...
$result    = aeopugmill_decrypt( $truncated );
...
$result2    = aeopugmill_decrypt( $bad_cipher );
assert_equal( 'garbage ciphertext returns empty string', '', $result2 );

// ── 7. aeopugmill_mask_secret ──────────────────────────────────────────────

section( '7. aeopugmill_mask_secret — display masking' );

assert_equal( 'empty value returns empty string',         '',             aeopugmill_mask_secret( '' ) );
assert_equal( 'short value (≤8 chars) fully masked',      '••••••••',     aeopugmill_mask_secret( 'abcdefgh' ) );
assert_equal( '13-char value: first 4 + 5 dots + last 4',  'sk-a•••••xyz0', aeopugmill_mask_secret( 'sk-abcdefxyz0' ) );
// Real API key shape: "sk-ant-api03-abc123xyz456"
$api_key_sample = 'sk-ant-api03-abc123xyz456';
$masked         = aeopugmill_mask_secret( $api_key_sample );
